/**
 * Shared utilities for tabs examples
 */

/**
 * Wait for the Tabs library to initialize before running a callback
 * @param {Function} callback - Function to run once Tabs is ready
 * @param {number} maxWait - Maximum time to wait in milliseconds (default: 2000)
 */
export function waitForTabs(callback, maxWait = 2000) {
  const startTime = Date.now();
  const checkInterval = 100;

  const check = () => {
    if (window.Tabs && window.Tabs.getAll && window.Tabs.getAll().length > 0) {
      callback();
    } else if (Date.now() - startTime < maxWait) {
      setTimeout(check, checkInterval);
    } else {
      console.warn('Tabs did not initialize in time');
      callback();
    }
  };
