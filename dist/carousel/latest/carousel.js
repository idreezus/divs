...
(function () {
  ...
  const SELECTORS = Object.fromEntries(Object.entries(SELECTOR_ATTRS).map(([k, v])=>[
          k,
          sel(v)
      ]));
  // CSS classes applied to elements
  const CLASSES = {
      SCROLLING: 'carousel-scrolling',
      DISABLED: 'carousel-nav-disabled',
      ACTIVE: 'carousel-item-active',
      MARKER_ACTIVE: 'carousel-marker-active',
  ...
  function calculateReferencePoint(scrollLeft, containerWidth, snapAlign, offsets = {}) {
  ...
  function findActiveIndex(itemPositions, scrollLeft, containerWidth, snapAlign, options = {}) {
      const { startInset = 0, endInset = 0 } = options;
      const referencePoint = calculateReferencePoint(scrollLeft, containerWidth, snapAlign, {
          startInset,
          endInset
      });
      let closestIndex = 0;
  ...
  function calculateDimensions(instance) {
      ...
      const paddingInlineStart = parseOffset(trackStyle.paddingInlineStart || trackStyle.paddingLeft);
      const paddingInlineEnd = parseOffset(trackStyle.paddingInlineEnd || trackStyle.paddingRight);
      const scrollPaddingInlineStart = parseOffset(trackStyle.scrollPaddingInlineStart || trackStyle.scrollPaddingLeft);
      const scrollPaddingInlineEnd = parseOffset(trackStyle.scrollPaddingInlineEnd || trackStyle.scrollPaddingRight);
      const startInset = scrollPaddingInlineStart.specified ? scrollPaddingInlineStart.value : paddingInlineStart.value;
      const endInset = scrollPaddingInlineEnd.specified ? scrollPaddingInlineEnd.value : paddingInlineEnd.value;
      ...
      const itemPositions = items.map((item)=>{
          ...
          const marginStartValue = parseFloat(itemStyle.scrollMarginInlineStart || itemStyle.scrollMarginLeft);
          const marginEndValue = parseFloat(itemStyle.scrollMarginInlineEnd || itemStyle.scrollMarginRight);
      ...
      if (maxScroll <= 0) {
          ...
          maxReachableIndex = findActiveIndex(itemPositions, maxScroll, containerWidth, config.align, {
              startInset,
              endInset
          });
      }
  ...
  function warnUnreachableItems(instance) {
      ...
      ].filter((x)=>Boolean(x)).join(' and ');
      console.warn(`Carousel ${id}: ${unreachableCount} item(s) (indices ${state.maxReachableIndex + 1}-${items.length - 1}) ` + `share the same scroll position as item ${state.maxReachableIndex} and cannot be individually activated. ` + `${features} will cycle through ${state.totalPositions} positions instead of ${items.length}. ` + `To make every item individually reachable, use wider items or add padding-inline-end to the track.`);
  }
  ...
  function startAutoplay(instance) {
      ...
      emit(instance, EVENTS.AUTOPLAY_START, {
  ...
  function resumeAutoplay(instance) {
      ...
      state.autoplayStartTime = sameItem ? performance.now() - (state.autoplayElapsed || 0) : performance.now();
      container.classList.add(CLASSES.PLAYING);
      // Update play/pause buttons
      instance.playPauseBtns.forEach((btn)=>btn.setAttribute('aria-label', 'Stop autoplay'));
      // Suppress live region announcements during autoplay
  ...
  function detectActiveItem(instance) {
      ...
      const scrollLeft = track.scrollLeft;
      const activeIndex = findActiveIndex(itemPositions, scrollLeft, state.containerWidth, snapAlign, {
          startInset,
          endInset
      });
      ...
      if (activeIndex !== currentIndex) {
          ...
          updateUI(instance);
          emit(instance, EVENTS.SNAPCHANGE, {
              index: activeIndex
          });
      }
  }
  ...
  function updateButtonStates(instance) {
      ...
      if (config.loop) {
          ...
          prevBtns.forEach((btn)=>{
              btn.classList.toggle(CLASSES.DISABLED, atFirstPosition);
              btn.disabled = atFirstPosition;
          });
          nextBtns.forEach((btn)=>{
              btn.classList.toggle(CLASSES.DISABLED, atLastPosition);
              btn.disabled = atLastPosition;
          });
      }
  ...
  function setupMarkers(instance) {
      ...
      allMarkers.forEach((marker, index)=>{
          ...
          marker.setAttribute('aria-label', `Scroll to item ${index + 1} of ${totalPositions}`);
          // Bind click handler (goTo handles autoplay stop)
          const handler = ()=>{
              instance.goTo(index);
          };
          marker.addEventListener('click', handler);
          instance.boundMarkerHandlers.push({
              marker,
              handler
          });
  ...
  function setupMarkerKeyboard(instance) {
      ...
      const handler = (event)=>{
          ...
          switch(event.key){
              case 'ArrowRight':
                  targetIndex = currentMarkerIndex < lastIndex ? currentMarkerIndex + 1 : config.loop ? 0 : lastIndex;
                  break;
              case 'ArrowLeft':
                  targetIndex = currentMarkerIndex > 0 ? currentMarkerIndex - 1 : config.loop ? lastIndex : 0;
                  break;
              case 'Home':
                  targetIndex = 0;
                  break;
  ...
  function updateMarkers(instance) {
      ...
      instance.counterCurrentEls.forEach((el)=>{
          el.textContent = String(currentIndex + 1);
      });
      instance.counterTotalEls.forEach((el)=>{
          el.textContent = String(state.totalPositions);
      });
  }
  ...
  function findElements(instance) {
      ...
      if (!track) {
          console.warn(`Carousel ${id}: Track element not found. Expected element with data-carousel-track.`);
          return false;
      }
      // Find required item elements (legacy backwards compat)
      const items = [
          ...container.querySelectorAll(`${SELECTORS.ITEM}, [data-carousel="item"]`)
      ];
      if (items.length === 0) {
          console.warn(`Carousel ${id}: No items found. Expected at least one element with data-carousel-item.`);
          return false;
      }
      // Find optional navigation buttons (legacy backwards compat)
      const prevBtns = [
          ...container.querySelectorAll(`${SELECTORS.PREV_BTN}, [data-carousel="prev"]`)
      ];
      const nextBtns = [
          ...container.querySelectorAll(`${SELECTORS.NEXT_BTN}, [data-carousel="next"]`)
      ];
      // Find optional markers (can be anywhere in container)
      const markers = [
          ...container.querySelectorAll(SELECTORS.MARKER)
      ];
      // Find optional counter elements
      const counterCurrentEls = [
          ...container.querySelectorAll(SELECTORS.COUNTER_CURRENT)
      ];
      const counterTotalEls = [
          ...container.querySelectorAll(SELECTORS.COUNTER_TOTAL)
      ];
      ...
      if (instance.config.autoplay) {
          playPauseBtns = [
              ...container.querySelectorAll(SELECTORS.PLAY_PAUSE_BTN)
          ];
          restartBtns = [
              ...container.querySelectorAll(SELECTORS.RESTART_BTN)
          ];
      } else {
          if (container.querySelector(SELECTORS.PLAY_PAUSE_BTN)) {
              console.warn(`Carousel ${id}: Play/pause button found but autoplay is not enabled. Add data-carousel-autoplay to the container.`);
          }
          if (container.querySelector(SELECTORS.RESTART_BTN)) {
              console.warn(`Carousel ${id}: Restart button found but autoplay is not enabled. Add data-carousel-autoplay to the container.`);
          }
      }
  ...
  function init(instance) {
      ...
      if (config.autoplay && !prefersReducedMotion()) {
          if (instance.state.totalPositions <= 1) {
              console.warn(`Carousel ${instance.id}: Autoplay has nothing to cycle through (only 1 item). Add more items or remove data-carousel-autoplay.`);
          }
  ...
  class Carousel {
      ...
      constructor(container){
          ...
          if (!initialized) {
              console.warn(`Carousel ${id}: Initialization failed due to missing required elements.`);
          }
      }
      ...
      goTo(targetIndex) {
          ...
          if (targetIndex < 0 || targetIndex >= items.length) {
              console.warn(`Carousel ${this.id}: Invalid index ${targetIndex}. Must be between 0 and ${items.length - 1}.`);
              return this;
          }
      ...
      play() {
          if (!this.config.autoplay) {
              console.warn(`Carousel ${this.id}: Autoplay is not enabled. Add data-carousel-autoplay to the container.`);
              return this;
          }
  ...
  function autoInit() {
      ...
      const containers = document.querySelectorAll(`${SELECTORS.CONTAINER}, [data-carousel="container"]`);
